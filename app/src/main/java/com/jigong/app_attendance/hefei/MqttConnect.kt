...
suspend fun getBasic(deviceNo: String) = withContext(Dispatchers.IO) {
        get(GlobalCode.TOPIC_PREFIX + deviceNo, deviceNo)
}
...
suspend fun pushBasicOnline(deviceNo: String) = withContext(Dispatchers.IO) {
        ...
        push(GlobalCode.BASIC_PUSH, JSON.toJSONString(getMap.await()), deviceNo)
}
...
suspend fun pushHeartbeat(deviceNo: String) = withContext(Dispatchers.IO) {
        push(GlobalCode.HEARTBEAT_PUSH, JSON.toJSONString(getHeartbeatDataMap(deviceNo)), deviceNo)
}
...
suspend fun pushBasicOffline(deviceNo: String) = withContext(Dispatchers.IO) {
        push(GlobalCode.BASIC_PUSH, JSON.toJSONString(getOfflineDataMap(deviceNo)), deviceNo)
}
...
suspend fun pushReplyWorkerInfo(messageId: String, sucNum: Int, errNum: Int, deviceNo: String) = withContext(Dispatchers.IO) {
        push(GlobalCode.TOPIC_PREFIX + deviceNo + "/Ack", JSON.toJSONString(getReplyGetWorkerInfoDataMap(deviceNo, messageId, sucNum, errNum)), deviceNo)
}
...
suspend fun pushAttendance(attendanceInfo: AttendanceInfo, deviceNo: String) = withContext(Dispatchers.IO) {
        ...
        * */
        ...
        push(GlobalCode.TOPIC_PREFIX + deviceNo + "/Rec", JSON.toJSONString(getMap.await()), deviceNo)
}