...
* 调用下发工人信息指令回复时上传的数据
* */
fun getReplyGetWorkerInfoDataMap(deviceNo: String, messageId: String, sucNum: Int, errNum: Int): Map<String, Any> {
    val infoMap = mapOf(
            Pair("facesluiceId", deviceNo),
...
fun getAttendanceDataMap(deviceNo: String, attendanceInfo: AttendanceInfo): Map<String, Any> {
    val infoMap = mapOf(
            ...
            Pair("customId", ""),//人员唯一 ID(最长 48 字符), 如果设备存在该 id,则为修改操作,否则为新增操作
            Pair("RecordID", ""),//记录 ID
            Pair("personName", "ok"),//人员姓名(最长 48 字符)
            Pair("idCard", "ok"),//证件号码
            Pair("Sendintime", "1"),//0: 非实时数据(不在 10 秒 内的考勤记录) 1: 实时数据(10 秒内的考勤记录)
            Pair("uniqueId", deviceNo),//设备的唯一 ID,每个设备要求不同(如固件 ID,cpuID,mac 地址等) (最长 128 字符)
            Pair("time", "ok"),//考勤时间 如 2022-07-06 10:02:16
            Pair("liveDetect", "ok"),//活体验证结果,T:通过 F:未通过 N:未开启
            Pair("direction", "ok"),//出入口方向进口:"enter" ，出口:"exit" ,无方向: "unknown"
            Pair("pic", "ok")//考勤照片(base64 图片, jpg 格式,最大 64K,分辨率最大 320x320 )

    )
    val operator = "RecPush-Ack"
    val sign = SignTest.getSign(0, PublicTopicAddress.BASIC_PUSH + operator + deviceNo)
    return mapOf(
            Pair("operator", operator),
            Pair("clientSign", sign),
